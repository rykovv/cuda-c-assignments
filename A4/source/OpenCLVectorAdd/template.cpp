...
#define LOCAL_WORK_SIZE 512U // block size in cuda
...
// Custom function for converting the OpenCL error codes to strings.
const char* get_error_string(cl_int err);

//@@ Write the OpenCL kernel
const char* kernelSource = "__kernel void vadd (__global const float *a, __global const float *b, __global float *result, int n) {\n"
...
int main(int argc, char* argv[]) {
    ...
    size_t n_work_groups = (inputLength - 1) / local_work_size + 1;
    ...
    wbCheck(clSetKernelArg(kernel, 3, sizeof(int),    (void*)&inputLength));
    //@@ Execute the kernel over the entire range of the data set
    cl_event event = NULL;
    wbCheck(clEnqueueNDRangeKernel(queue, kernel, 1, NULL, &global_work_size, &local_work_size, 0, NULL, &event));
